## Version 1.0.7 (2026073000)
* New Feature: Treefilter for entities.
* Improvement: Accessibility (WCAG) for the [sparten] shortcode: the sports category and sport accordion toggles are now keyboard-operable buttons inside ARIA headings (divs with role="heading" and aria-level, so no h1-h6 tags are used and the heading sizes stay exactly as before), aria-expanded reflects the actual initial state, the substitution pool collapses got unique IDs (the category-level "mail problems" link toggled the wrong element before), substitution teacher tables got proper column headers for screen readers and decorative icons are hidden from assistive technology.
...
* Bugfix: The site setting "Turn off modals" (booking | turnoffmodals) now works in the list shortcodes ([allekurseliste], [meinekurseliste], ...). Three things prevented it: musi_table passed neither the inline start page nor the rendered view on to mod_booking, so the view configured in the booking instance decided and a cards instance kept opening modals; the list row template was missing the mod-booking-row class the inline pre booking pages are appended to, so they were never loaded; and the cached button HTML was not keyed by the rendered view, so a list and a cards shortcode of the same booking option shared one cache entry and whichever page was opened first won.
* Bugfix: The grid shortcode ([allekursegrid]) now reports the cards view to mod_booking as well - a grid entry is as narrow as a card and cannot host the inline pre booking pages either.
